...
function createClient(bin, extra) {
  ...
  function standalone(root) {
    if (!root)
      return true;
    if ((0, import_fs.existsSync)((0, import_path.join)(import_coc.Uri.parse(root).fsPath, "Cargo.toml")))
      return false;
    if ((0, import_fs.existsSync)((0, import_path.join)(import_coc.Uri.parse(root).fsPath, "rust-project.json")))
      return false;
    return true;
  }
  let initializationOptions = import_coc.workspace.getConfiguration("rust-analyzer");
  if (standalone((_b = import_coc.workspace.workspaceFolder) == null ? void 0 : _b.uri)) {
    const docs = import_coc.workspace.documents.filter((doc) => isRustDocument(doc.textDocument));
    if (docs.length) {
      initializationOptions = __spreadValues({ detachedFiles: docs.map((doc) => import_coc.Uri.parse(doc.uri).fsPath) }, initializationOptions);
    }
  }
  ...
  const clientOptions = {
    ...
    middleware: {
      async provideHover(document2, position, token) {
        let positionOrRange = null;
        const mode = await import_coc.workspace.nvim.call("mode");
        if (mode === "v" || mode === "V") {
          const doc = import_coc.workspace.getDocument(document2.uri);
          await import_coc.workspace.nvim.call("eval", 'feedkeys("\\<esc>", "in")');
          positionOrRange = await import_coc.workspace.getSelectedRange(mode, doc);
        }
        if (!positionOrRange)
        ...
        return await client.sendRequest(hover, param, token);
      },
...
var Config = class {
  constructor() {
    ...
    this.requiresReloadOpts = ["server", "cargo", "procMacro", "files", "updates", "lens", "inlayHints"].map((opt) => `${this.rootSection}.${opt}`);
  ...
  get checkOnStartup() {
    return this.cfg.get("updates.checkOnStartup");
  }
  get terminal() {
    return {
...
`;
  ...
  await import_fs2.promises.rename(dest, origFile);
  ...
  await import_fs2.promises.unlink(origFile);
}
function isMusl() {
...
async function downloadServer(context, release) {
  ...
  const _path = import_path2.default.join(context.storagePath, release.name);
  const randomHex = (0, import_crypto.randomBytes)(5).toString("hex");
  const tempFile = import_path2.default.join(context.storagePath, `${release.name}${randomHex}`);
  ...
  try {
    if (await import_fs2.promises.stat("/etc/nixos")) {
...
function isCargoTomlDocument(document2) {
  const u = import_coc5.Uri.parse(document2.uri);
  ...
  async startServer() {
    ...
    client.onNotification(serverStatus, async (status) => {
      ...
      if (status.health !== "ok" && ((_a = status.message) == null ? void 0 : _a.length)) {
        if (status.message.startsWith("cargo check failed"))
          return;
...
function parentModule2(ctx) {
  return async () => {
    var _a, _b;
    const { document: document2, position } = await import_coc6.workspace.getCurrentState();
    if (!(isRustDocument(document2) || isCargoTomlDocument(document2)))
      return;
    const param = {
      textDocument: { uri: document2.uri },
      position
...
function debugSingle(ctx) {
  return async (runnable) => {
    ...
    if (runtime === "termdebug") {
      await import_coc6.workspace.nvim.command(`TermdebugCommand ${executable} ${executableArgs}`);
      return;
    }
...
function explainError(ctx) {
  return async () => {
    ...
    if (diagnostic == null ? void 0 : diagnostic.code) {
      const explanation = (0, import_child_process2.spawnSync)("rustc", ["--explain", `${diagnostic.code}`], { encoding: "utf-8" }).stdout.toString();
      const docs = [];
      let isCode = false;
...
function resolveCodeAction(ctx) {
  return async (params) => {
    var _a;
    const item = await ctx.client.sendRequest(import_vscode_languageserver_protocol3.CodeActionResolveRequest.method, params);
...
function crateGraph(ctx, full) {
  return async () => {
    const dot = await ctx.client.sendRequest(viewCrateGraph, { full });
    const html = `
<!DOCTYPE html>
<meta charset="utf-8">
<head>
...
`;
    const tempFile = (0, import_path4.join)((0, import_os2.tmpdir)(), `${(0, import_crypto2.randomBytes)(5).toString("hex")}.html`);
    (0, import_fs4.writeFileSync)(tempFile, html, { encoding: "utf-8" });
    await import_coc6.workspace.nvim.call("coc#util#open_url", [tempFile]);
  };
}
function viewCrateGraph2(ctx) {
  return crateGraph(ctx, false);
}
function viewFullCrateGraph(ctx) {
  return crateGraph(ctx, true);
}
function viewItemTree2(ctx) {
  return async () => {
    const { document: document2 } = await import_coc6.workspace.getCurrentState();
    if (!isRustDocument(document2))
      return;
    ...
    const ret = await ctx.client.sendRequest(viewItemTree, param);
    if (!ret)
      return;
    const nvim = import_coc6.workspace.nvim;
    nvim.pauseNotification();
...
async function activate(context) {
  ...
  if (!(0, import_fs5.existsSync)(serverRoot)) {
    (0, import_fs5.mkdirSync)(serverRoot);
  }
  const bin = ctx.resolveBin();
  if (!bin) {
  ...
  ctx.registerCommand("debug", debug);
  ...
  ctx.registerCommand("viewItemTree", viewItemTree2);
  ...
  ctx.registerCommand("viewCrateGraph", viewCrateGraph2);
  ctx.registerCommand("viewFullCrateGraph", viewFullCrateGraph);
